<?php

  $teamBlueFull       = 'Gypsy Danger';
  $teamBlueFirst      = 'Gypsy';
  $teamBlueLast       = 'Danger';

  $teamBlackFull      = 'Here 4 Beer';
  $teamBlackFirst     = 'Here';
  $teamBlackLast      = '4 Beer';

  $teamRedFull        = 'Team Red';
  $teamRedFirst       = 'Team';
  $teamRedLast        = 'Red';

  $teamGreyFull       = 'Team Grey';
  $teamGreyFirst      = 'Team';
  $teamGreyLast       = 'Grey';

  $teamYellowFull     = 'Team Yellow';
  $teamYellowFirst    = 'Team';
  $teamYellowLast     = 'Yellow';

  $teamTanFull        = 'Fat Baby Rohans';
  $teamTanFirst       = 'Fat Baby';
  $teamTanLast        = 'Rohans';

  $teamLightBlueFull  = 'Team Light Blue';
  $teamLightBlueFirst = 'Team';
  $teamLightBlueLast  = 'Light Blue';

  $teamGreenFull      = 'Team Green';
  $teamGreenFirst     = 'Team';
  $teamGreenLast      = 'Green';

  $teamMaroonFull     = 'Team Maroon';
  $teamMaroonFirst    = 'Team';
  $teamMaroonLast     = 'Maroon';

  $teamWhiteFull      = 'Team White';
  $teamWhiteFirst     = 'Team';
  $teamWhiteLast      = 'White';

  $teamTBD            = 'TBD';

?>
